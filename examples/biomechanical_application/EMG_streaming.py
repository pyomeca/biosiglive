...
def get_custom_function(device_interface):
    custom_processing = RealTimeProcessing(
        data_rate=device_interface.get_device(name="emg").rate, processing_windows=1000
    )
...
if __name__ == "__main__":
    ...
    # Add plot
    emg_plot = LivePlot(
        name="emg", channel_names=["raw_data", "custom_emg_process", "proc_emg"], plot_type="curve", nb_subplots=3
    )
    emg_plot.init(
    ...
    while True:
        ...
        emg_proc_custom = interface.devices[0].process(
            method=RealTimeProcessingMethod.Custom,
